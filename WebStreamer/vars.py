...
class Var(object):
    ...
    WORKERS = int(environ.get("WORKERS",
                              "6"))  # 6 workers = 6 commands at once
    BIN_CHANNEL = int(environ.get(
        "BIN_CHANNEL",
        None))  # you NEED to use a CHANNEL when you're using MULTI_CLIENT
    ...
    AUTOHORISED = str(environ.get("AUTHORISED"))
    DETA = [int(xd) for xd in AUTOHORISED.split(",")]
    HASH0 = str(environ.get("HASH"))
    HASH = [yd for yd in HASH0.split(",")]
    if "DYNO" in environ:
        ON_HEROKU = True
